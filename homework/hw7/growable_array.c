...
void garray_print(garray_t* garray, void (*print_fn)(void*)) {
  for (unsigned int i = 0; i < garray->num_vals; i++) {
    void* val = garray->data[i];
    (*print_fn)(val);
  }
}
...
void garray_apply_to_element(garray_t* garray, void* (*element_fn)(void*)) {
  for (unsigned int i = 0; i < garray->num_vals; i++) {
    void* val = garray->data[i];
    void* new_val = (*element_fn)(val);
    garray->data[i] = new_val;
  }
}
...
garray_t* garray_map(garray_t* garray, 
                     ...
                     void* extra_arg) {
  // the result will have the same number of values
  // as the input garray.  Might as well start with
  // the right size.
  garray_t* rv = mk_garray(garray_get_length(garray));
  for (unsigned int i = 0; i < garray->num_vals; i++) {
    void* val = garray->data[i];
    void* new_val = (*element_fn)(val, extra_arg);
    garray_append(rv, new_val);
  }
  return rv;
}

void garray_free_with_elements(garray_t* garray,
                               void (*element_free_fn)(void*)) {
  for (unsigned int i = 0; i < garray->num_vals; i++) {
    void* val = garray->data[i];
    (*element_free_fn)(val);
  }
  // free up the infrastructure.
  garray_free(garray);
}
