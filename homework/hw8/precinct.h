 ...
 * Returns: nothing
 */
void add_ballot_to_precinct(precinct_t *precinct, char **ballot);
...
/**** NEW in Homework 8 ****/
/* count_votes_precinct: count the votes for the precinct.
 *
 * precinct: the precinct of interest
 * candidates: the viable candidates
 * num_exhausted_ballots: an OUT parameter for returning the number of exhaused ballots
 *
 * Returns: an array where the ith entry is votes earned by the ith
 *   candidate (that is, the ith name in the names array in the viable
 *   candidates structure).
 */
unsigned int *count_votes_precinct(precinct_t *precinct,
				   candidates_t *viable_candidates,
